// Dados simulados de peneiras de futebol - VERSÃO COM CEP
const peneirasData = [
    {
        id: 1,
        titulo: "Peneira Sub-15 e Sub-17",
        clube: "Santos FC",
        cep: "11075-500", // Vila Belmiro, Santos, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-08-15",
        horario: "14:00",
    ...
    },
    {
        ...
        clube: "São Paulo FC",
        cep: "01124-060", // Barra Funda, São Paulo, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-08-20",
        horario: "09:00",
    ...
    },
    {
        ...
        clube: "Corinthians",
        cep: "08295-005", // Itaquera, São Paulo, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-08-25",
        horario: "15:30",
    ...
    },
    {
        ...
        clube: "Palmeiras",
        cep: "05466-000", // Barra Funda, São Paulo, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-09-01",
        horario: "10:00",
    ...
    },
    {
        ...
        cep: "12916-440", // Bragança Paulista, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-09-05",
        horario: "13:00",
    ...
    },
    {
        ...
        clube: "Ponte Preta",
        cep: "13010-111", // Centro, Campinas, SP
        endereco: null, // Será preenchido dinamicamente
        data: "2024-09-10",
        horario: "14:30",
        categoria: "Sub-13, Sub-15",
...
async function buscarEnderecoPorCEP(cep) {
    ...
    try {
        ...
        // Armazenar no cache
        enderecoCache.set(cep, endereco);
...
// Função para carregar endereços de todas as peneiras
async function carregarEnderecosPeneiras() {
    const promises = peneirasData.map(async (peneira) => {
        if (!peneira.endereco && peneira.cep) {
            try {
                peneira.endereco = await buscarEnderecoPorCEP(peneira.cep);
            } catch (error) {
                console.error(`Erro ao carregar endereço para peneira ${peneira.id}:`, error);
                peneira.endereco = 'Endereço não disponível';
            }
        }
    });
...
function initializeApp() {
    // Carregar endereços das peneiras na inicialização
    carregarEnderecosPeneiras().then(() => {
        console.log('Endereços das peneiras carregados com sucesso');
    }).catch(error => {
        console.error('Erro ao carregar endereços das peneiras:', error);
    });
...
async function searchPeneiras(location) {
    try {
...
async function displayResults(results) {
    ...
    for (let i = 0; i < results.length; i++) {
        const peneira = results[i];
        const resultCard = await createResultCard(peneira);
...
async function createResultCard(peneira) {
    const card = document.createElement('div');
    card.className = 'result-card';
    
    // Garantir que o endereço esteja carregado
    if (!peneira.endereco && peneira.cep) {
        peneira.endereco = await buscarEnderecoPorCEP(peneira.cep);
    }
        ...
        <div class="card-content">
            <div class="event-details">
                ...
                <div class="detail-row">
                    ...
                    <span>${peneira.endereco || 'Carregando endereço...'}</span>
                </div>
                <div class="detail-row">
